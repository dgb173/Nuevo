...
def parse_main_page_matches(html_content, limit=50, offset=0, handicap_filter=None):
...
async def get_main_page_matches_async(limit=50, offset=0, handicap_filter=None):
...
def proximos():
    try:
        ...
        hf = request.args.get('handicap')
        matches = asyncio.run(get_main_page_matches_async(50, 0, hf))
        print(f"Scraper finalizado. {len(matches)} partidos encontrados.")
        opts = sorted({
            normalize_handicap_to_half_bucket_str(m.get('handicap'))
            for m in matches if normalize_handicap_to_half_bucket_str(m.get('handicap')) is not None
